"""
main_2.py — основная точка входа Telegram-бота NutriSnap (@nutrisnap_ultra_bot).

Зачем нужен файл
----------------
Полный FSM-флоу учёта калорий: пользователь присылает фото блюда/этикетки
(опционально с подписью) или текстовое описание → Gemini оценивает КБЖУ и вес
порции → бот показывает превью с кнопками → пользователь подтверждает, правит
или ждёт автоподтверждение → итог печатается JSON в консоль (заглушка дневника).
Запуск: python main_2.py (long polling через aiogram 3.x).

Как устроен файл (блоки сверху вниз, как в коде)
------------------------------------------------

1. Импорты и зависимости
   aiogram (Bot, Dispatcher, FSM, клавиатуры), google.genai, pydantic, dotenv.

2. Конфиг
   Ключи из .env (TELEGRAM_BOT_API_KEY, GEMINI_API_KEY), таймауты запросов к
   модели и автоподтверждения, стандартная порция 100 г, очередь моделей
   Gemini с fallback, константы callback_data и тексты reply-кнопок меню правок.

3. Промпты
   PHOTO_PROMPT — инструкции для анализа фото (ветки status: recognized /
   unclear / no_food / label / bad_desc).
   TEXT_PROMPT — то же для чистого текстового описания без фото.

4. Схема ответа и инфраструктура бота
   Клиент Gemini, MemoryStorage, Dispatcher.
   FoodFlow — FSM-состояния: confirming, editing_choice, waiting_hint,
   waiting_weight.
   FoodResult — Pydantic-схема structured output (status, dish, calories,
   БЖУ, portion_g, portion_known, is_label).

5. Анализ через Gemini
   _generate_with_fallback — общий вызов модели с JSON-схемой и перебором
   MODELS_QUEUE при ошибке/таймауте.
   analyze_food_photo — загрузка фото в Files API + промпт (+ опциональная
   текстовая подсказка), затем удаление файла из облака.
   analyze_food_text — анализ текстового описания без фото.

6. Результат (нормализация и показ)
...
7. Клавиатуры
   ...
   Reply: меню правок после «✏️ Изменить» (вес / уточнить / заменить / отменить).

8. Утилиты
   save_to_console — «сохранение» результата (print JSON вместо БД).
   download_photo_temp — скачивание Telegram-фото во временный файл.
   parse_food_result — разбор сырого JSON ответа модели в FoodResult.
...
9. Confirm UI (логика подтверждения)
   ...
   bad_desc / unclear (просит подсказку) / recognized|label (превью) /
   ошибка.

10. Обработчики сообщений (по порядку регистрации)
    /start — приветствие и краткая инструкция.
    Фото — анализ через Gemini, дальше handle_ai_result.
    Текст в waiting_hint — уточнение: повторный анализ фото с подсказкой
    или пересчёт чисто текстового результата.
    Текст в waiting_weight — ввод граммов, пересчёт КБЖУ, сохранение.
    Обычный текст (вне спец. состояний / в confirming) — текстовая ветка
    учёта еды → анализ → превью (unclear/bad_desc для текста — мягкие
    ответы без FSM waiting_hint).

11. Callbacks и меню правок
    ✅ — сохранить в консоль, сбросить FSM, инвалидировать таймер.
    ✏️ — показать reply-меню editing_choice.
    Пункты меню: изменить вес → waiting_weight; дополнить описание →
    waiting_hint; заменить ввод — сброс и ожидание нового фото/текста;
    не сохранять — выход без записи; прочий текст — напоминание выбрать
    кнопку.
    ⚖️ — ввод точного веса (этикетка).
    «Отмена» — выход из waiting_hint.

12. Запуск
    main() проверяет наличие ключей и стартует polling; при Ctrl+C —
